...
class OpenAIChatShape:
    ...
    def complete(self, j):
        ...
        out = {"id": self.id, "object": "chat.completion", "created": self.created, "model": j.get("model"),
               "system_fingerprint": "fp_skirnir_router",
    ...
    def _chunk(self, model, delta, finish=None):
        return {"id": self.id, "object": "chat.completion.chunk", "created": self.created, "model": model,
                "system_fingerprint": "fp_skirnir_router", "choices": [{"index": 0, "delta": delta, "finish_reason": finish}]}
...
class OpenAICompletionShape(OpenAIChatShape):
    ...
    def chunk(self, j):
        base = {"id": self.id, "object": "text_completion", "created": self.created, "model": j.get("model"), "system_fingerprint": "fp_skirnir_router"}
        out = b""
        if j.get("response"):
