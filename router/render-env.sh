#!/bin/bash
# ollama-router: Laufzeit-Secrets aus dem Secret-Store rendern (Universal-Auth-API).
# Zugang steht NICHT hier, sondern in /etc/ollama-router/render-env.conf (deploy.py legt sie aus dem Ops-Ordner ab, 0600):
...
# Schreibt /etc/ollama-router/secrets.env (600), das ollama-router.service als EnvironmentFile liest.
# Bei Ausfall des Secret-Stores bleibt die letzte Datei stehen.
# Aufruf ohne Argument (als Abhaengigkeit von ollama-router.service): NUR rendern, NIE den Router anfassen -
...
if [ ! -f "$OUT" ] || ! cmp -s "$TMP" "$OUT"; then
  ...
  if [ "$RESTART" = 1 ] && systemctl is-active --quiet ollama-router; then
    systemctl restart --no-block ollama-router && echo "ollama-router Neustart angestossen (Secret geaendert)"
  fi
else
